...
- [design-unilab-asset-delivery.md](../design-unilab-asset-delivery.md)
...
| AssetDelivery はスコープベースのハンドル管理（`IAssetScope`） | 個別 Release は漏れる。VContainer の Scoped Dispose に紐付けて構造的に解決 |
...
- `UniLab.Integration` の条件コンパイル矛盾 → 「Addressables 未導入なら AssetDelivery / Integration をフォルダごと持ち込まない」運用に確定
...
2. 実装は Popup v2 → AssetDelivery → IAP の順（overview 参照）
3. Popup v2 着手時: `IPopupService` / `IPopupViewProvider` のインターフェース定義から
4. manifest.json への `com.unity.addressables` 追加は AssetDelivery 着手時、`com.unity.purchasing` は IAP 着手時
