...
## 5b. シナリオランナーの待ち方 — 条件で待ち、フレーム数で待たない

### 問題

`settleFrames` は静止画のための概念だった。「動きが収まってから1枚撮る」なら余白は写らない。
動画にした瞬間、この余白がそのまま尺になって露出し、**ドライバの待ちがゲームの遅さに見える**事故が起きた
（実測: 遷移そのものは 0.6 秒だったが、動画では前後に 14 秒の静止が写り「遷移が遅い」と誤認された）。

観測器が観測対象の見え方を変えてはいけない。

### 方式

**対象が「プレイヤーが押せる状態」になった瞬間に送出する。** 条件は3つで、すべて満たしたフレームで即操作する。

| 条件 | 判定 | 何を待てるか |
|---|---|---|
| 存在する | 名前パスで見つかる | 生成待ち・シーン到着 |
| 遮られていない | 対象の中心へレイキャストし、最前面が自身か子孫 | モーダルの暗幕・遷移フェード |
| 操作可能 | `Selectable.IsInteractable()`（親 CanvasGroup を含む） | 開くアニメーション中の不活性 |

3つめが要る理由: 最前面にあっても、ポップアップが開くアニメーション中は CanvasGroup が
`interactable=false` で、`Button.OnSubmit` はその入力を**黙って捨てる**。
レイキャストだけを条件にすると、この瞬間に押して何も起きない（実測で再現した）。

`Selectable` を持たない対象は3つめを判定できないため、存在と遮蔽だけで判断する。

### 待ちの意味を分ける

| 待ち | 意味 | 既定 |
|---|---|---|
| 準備待ち（条件） | ゲームが操作を受け付けるまで | 常に行う。上限 30 秒（実時間） |
| `waitScene` | **操作した結果の**シーン到着を待つ | 指定時のみ。操作の後に待つ |
| `settleFrames` | 操作後、絵が収まるまで待つ | 撮影・監査があるステップだけ 30。それ以外は 0 |

`waitScene` は操作の**後**に待つ。前に待つと「操作しないと始まらない遷移」を永遠に待つ
（実装時にこれを逆にして詰まった）。

### 遮られている間は送出しない

以前は「遮られていても警告を出して送出する」だったが、条件待ちでは**押せる状態になるまで待ち、
上限を超えたら送出を見送る**。モーダル越しに裏のボタンを押す事故はこれで構造的に消えた。

### 副産物: 応答時間が計測値になる

マーカーの `waited` に「押せる状態になるまで待った実時間」が入る。
これはドライバの余白を含まない、**ゲームの応答時間そのもの**である。

```
step3 submit=NewGameButton  waited=0.20s   ← 起動フェードが明けるまで
step4 submit=MessageButton0 waited=0.20s   ← ダイアログが開いて押せるまで
step6 submit=TabButton1     waited=0.23s   ← シーン遷移のフェードが明けるまで
step7 submit=TabButton2     waited=0.03s   ← 即応（1フレーム）
```

### 露出した潜在不具合

条件待ちに切り替えた直後、既存の全画面巡回シナリオで**押せないボタンを押していた箇所**が露出した。
編成タブの `CoreSlotButton` は #66 で素体変更が人形タブへ移った際に常時 disabled になっていたが、
旧ランナーは `Button` が入力を捨てるのを検知できず、撮影した「候補一覧」は編成画面の複製だった。
**数週間、偽の成功が続いていた。** 条件待ちは「押したつもり」を成立させないため、この種の腐敗を即座に表面化させる。

## 5c. 録画に入力を写す（→ 設計書 11）

録画の主用途は「見て確認する」ことであり、**そのとき何を押したかが写っていない動画は診断に使えない**。
入力可視化オーバーレイ（`design-unilab-ai-11-input-overlay.md`）は録画開始時に既定で有効になり、
押したボタン・ポインタ・クリック・タッチと現在のステップを画面に描く。詳細はそちらを参照。
